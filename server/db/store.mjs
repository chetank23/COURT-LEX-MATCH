...
const memoryState = {
  judges: [
    { id: "judge-1", name: "Justice N. Rao",     courtLevel: "Supreme Court",  category: "Criminal", yearsOfExperience: 20, caseLoadCapacity: 50, currentCaseLoad: 38, availability: "Busy",       district: "New Delhi", state: "Delhi",       specializations: ["Criminal","Constitutional"] },
    { id: "judge-2", name: "Justice P. Mehta",   courtLevel: "High Court",     category: "Criminal", yearsOfExperience: 15, caseLoadCapacity: 60, currentCaseLoad: 42, availability: "Available",   district: "New Delhi", state: "Delhi",       specializations: ["Criminal"] },
    { id: "judge-3", name: "Justice R. Iyer",    courtLevel: "High Court",     category: "Civil",    yearsOfExperience: 18, caseLoadCapacity: 55, currentCaseLoad: 28, availability: "Available",   district: "Mumbai",    state: "Maharashtra", specializations: ["Civil","Commercial"] },
    { id: "judge-4", name: "Justice K. Banerjee",courtLevel: "High Court",     category: "Civil",    yearsOfExperience: 22, caseLoadCapacity: 50, currentCaseLoad: 31, availability: "Available",   district: "Kolkata",   state: "West Bengal", specializations: ["Civil","Labor"] },
    { id: "judge-5", name: "Justice S. Khan",    courtLevel: "District Court", category: "Criminal", yearsOfExperience: 10, caseLoadCapacity: 70, currentCaseLoad: 55, availability: "Busy",       district: "Hyderabad", state: "Telangana",   specializations: ["Criminal"] },
    { id: "judge-6", name: "Justice V. Sen",     courtLevel: "High Court",     category: "Civil",    yearsOfExperience: 16, caseLoadCapacity: 55, currentCaseLoad: 20, availability: "Available",   district: "Chennai",   state: "Tamil Nadu",  specializations: ["Civil","Revenue"] },
    { id: "judge-7", name: "Justice A. Menon",   courtLevel: "District Court", category: "Other",    yearsOfExperience: 12, caseLoadCapacity: 65, currentCaseLoad: 48, availability: "Available",   district: "Kochi",     state: "Kerala",      specializations: ["Constitutional","Labor"] },
    { id: "judge-8", name: "Justice D. Kapoor",  courtLevel: "District Court", category: "Other",    yearsOfExperience: 8,  caseLoadCapacity: 70, currentCaseLoad: 35, availability: "Available",   district: "Ahmedabad", state: "Gujarat",     specializations: ["Commercial","Revenue"] },
    { id: "judge-9", name: "Justice T. Joseph",  courtLevel: "District Court", category: "Other",    yearsOfExperience: 14, caseLoadCapacity: 60, currentCaseLoad: 40, availability: "On Leave",    district: "Bengaluru", state: "Karnataka",   specializations: ["Constitutional"] },
  ],
  hearings: [
    { id: "hearing-1", caseId: "case-101", caseTitle: "State vs. Ram Kumar", assignedJudgeId: "judge-1", assignedJudgeName: "Justice N. Rao", hearingDate: "2025-06-15", hearingTime: "10:00", courtRoom: "Court Room 1", state: "Delhi", district: "New Delhi", localCourtName: "Delhi High Court", status: "Scheduled", notes: "First hearing on merits" },
    { id: "hearing-2", caseId: "case-202", caseTitle: "Sharma & Co. vs. State", assignedJudgeId: "judge-3", assignedJudgeName: "Justice R. Iyer", hearingDate: "2025-06-20", hearingTime: "14:00", courtRoom: "Court Room 3", state: "Maharashtra", district: "Mumbai", localCourtName: "Bombay High Court", status: "Scheduled", notes: "Arguments on admissibility" },
  ],
  managedCases: [
    { id: "case-managed-1", title: "State vs Kumar (Public Safety Review)", status: "Assigned", assignedJudge: "Justice N. Rao", uploadedBy: "Court Support Officer", notes: "Urgent listing requested by prosecution.", autoAssigned: true, assignmentReason: "Selected for criminal bench fit.", priorityScore: 88, priorityBand: "P0", bailRiskScore: 79, escapeRiskScore: 72, riskScore: 76, publicDefenderStatus: "Pending Allocation", updatedAt: Date.now() },
    { id: "case-managed-2", title: "Anita Sharma vs Metro Developers", status: "Under Review", assignedJudge: "Justice R. Iyer", uploadedBy: "Court Support Officer", notes: "Awaiting affidavit verification.", autoAssigned: false, assignmentReason: "Manual assignment retained.", priorityScore: 62, priorityBand: "P2", bailRiskScore: 24, escapeRiskScore: 18, riskScore: 21, publicDefenderStatus: "Not Required", updatedAt: Date.now() },
  ],
  history: [],
};
...
function getDb() {
  ...
  try {
    const dbPath = process.env.DATABASE_PATH ||
      path.join(ROOT, "data", "lexmatch.db");
    ...
    dbAvailable = false;
    console.warn("⚠️  [store] SQLite unavailable — using in-memory store.", err.message);
    return null;
  }
}
...
function mapManagedCaseRow(row) {
  return {
    ...
    priorityScore: row.priority_score != null ? Number(row.priority_score) : undefined,
    priorityBand: row.priority_band || undefined,
    bailRiskScore: row.bail_risk_score != null ? Number(row.bail_risk_score) : undefined,
    escapeRiskScore: row.escape_risk_score != null ? Number(row.escape_risk_score) : undefined,
...
export async function createJudge(input) {
  ...
  if (database) {
    database.prepare(
      ...
      judge.id, judge.name, judge.courtLevel, judge.category, judge.yearsOfExperience,
      judge.caseLoadCapacity, judge.currentCaseLoad, judge.availability,
      judge.district, judge.state, judge.area, judge.courtName,
...
export async function updateJudge(id, updates) {
  ...
  if (database) {
    database.prepare(
      ...
      merged.name, merged.courtLevel, merged.category, merged.yearsOfExperience,
      merged.caseLoadCapacity, merged.currentCaseLoad, merged.availability,
      merged.district || null, merged.state || null, merged.area || null,
      merged.courtName || null, JSON.stringify(merged.specializations || []), id
    );
    return (await getJudgeById(id)) || merged;
  }
  memoryState.judges = memoryState.judges.map((j) => (j.id === id ? merged : j));
  return merged;
}
...
export async function listHearings(filters = {}) {
  ...
  if (database) {
    ...
    if (filters.judgeId) { clauses.push("assigned_judge_id=?");  params.push(filters.judgeId); }
  ...
  return memoryState.hearings
    .filter((h) => {
      ...
      if (filters.judgeId && h.assignedJudgeId !== filters.judgeId)  return false;
      return true;
    })
    .sort((a, b) => `${a.hearingDate}T${a.hearingTime}`.localeCompare(`${b.hearingDate}T${b.hearingTime}`));
}
...
export async function createHearing(input) {
  ...
  if (database) {
    database.prepare(
      ...
      hearing.id, hearing.caseId, hearing.caseTitle, hearing.assignedJudgeId,
      hearing.assignedJudgeName, hearing.hearingDate, hearing.hearingTime,
      hearing.courtRoom, hearing.state, hearing.district, hearing.localCourtName,
      hearing.status, hearing.notes
    );
...
export async function updateHearing(id, updates) {
  ...
  if (database) {
    database.prepare(
      ...
      merged.caseId, merged.caseTitle, merged.assignedJudgeId, merged.assignedJudgeName,
      merged.hearingDate, merged.hearingTime, merged.courtRoom, merged.state,
      merged.district, merged.localCourtName, merged.status, merged.notes, id
    );
    return (await getHearingById(id)) || merged;
  }
  memoryState.hearings = memoryState.hearings.map((h) => (h.id === id ? merged : h));
  return merged;
}
...
export async function listHistory() {
  ...
  return memoryState.history.slice().sort((a, b) => `${b.date}`.localeCompare(`${a.date}`));
}
  ...
  if (database) {
    database.prepare(
      `INSERT INTO activity_history (id, type, title, event_date, results, metadata)
       VALUES (?,?,?,?,?,?)`
    ).run(event.id, event.type, event.title, event.date, event.results, JSON.stringify(input.metadata || {}));
    return event;
  }
...
export async function upsertManagedCase(input) {
  const mc = {
    ...
    priorityScore: Number.isFinite(input.priorityScore) ? input.priorityScore : null,
    priorityBand: input.priorityBand || null,
    bailRiskScore: Number.isFinite(input.bailRiskScore) ? input.bailRiskScore : null,
    escapeRiskScore: Number.isFinite(input.escapeRiskScore) ? input.escapeRiskScore : null,
  ...
  if (database) {
    database.prepare(
      ...
      mc.id, mc.title, mc.status, mc.assignedJudge, mc.uploadedBy, mc.uploadName, mc.notes,
      mc.autoAssigned, mc.assignmentReason, mc.priorityScore, mc.priorityBand,
      mc.bailRiskScore, mc.escapeRiskScore, mc.riskScore, mc.publicDefenderStatus
    );
    const row = database.prepare("SELECT * FROM managed_cases WHERE id=?").get(mc.id);
    return row ? mapManagedCaseRow(row) : { ...mc, updatedAt: Date.now() };
  }

  const idx = memoryState.managedCases.findIndex((c) => c.id === mc.id);
  const full = { ...mc, autoAssigned: Boolean(mc.autoAssigned), updatedAt: Date.now() };
...
export async function updateManagedCase(id, updates) {
  ...
  let current;
  if (database) {
    const row = database.prepare("SELECT * FROM managed_cases WHERE id=?").get(id);
...
export async function deleteManagedCase(id) {
  const database = getDb();
  if (database) {
    const info = database.prepare("DELETE FROM managed_cases WHERE id=?").run(id);
    return info.changes > 0;
  }
  const prev = memoryState.managedCases.length;
  memoryState.managedCases = memoryState.managedCases.filter((c) => c.id !== id);
...
export async function createPriorityOverride(input) {
  ...
  if (database) {
    const info = database.prepare(
      ...
      input.caseId, input.originalBand || null, input.overrideBand,
      input.originalScore || null, input.overrideScore || null,
      input.reason, input.overriddenBy || "staff"
    );
    return {
...
export async function checkDbHealth() {
  try {
    const database = getDb();
    if (!database) return { ok: false, mode: "memory", message: "SQLite unavailable or not initialized" };
