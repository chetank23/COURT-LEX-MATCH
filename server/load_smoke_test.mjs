...
async function main() {
  ...
  try {
    ...
    const analyzeRequests = Array.from({ length: 10 }, () =>
      timedFetch(`${base}/api/analyze-pdf`, {
        ...
        body: JSON.stringify({
          ...
          extractedText: "Article 21 liberty interim injunction constitutional challenge.",
        }),
      })
    );
    ...
    assert.ok(searchResults.every((result) => result.status === 200), "Search load run returned non-200 responses");
    assert.ok(analyzeResults.every((result) => result.status === 200), "Analyze load run returned non-200 responses");
    ...
    assert.ok(searchStats.p95 < 8000, `Search p95 too high: ${searchStats.p95.toFixed(2)}ms`);
    assert.ok(analyzeStats.p95 < 5000, `Analyze p95 too high: ${analyzeStats.p95.toFixed(2)}ms`);
