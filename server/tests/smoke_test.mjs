...
async function main() {
  ...
  try {
    ...
    const search = await fetchJson(
      "http://127.0.0.1:4101/api/cases/search?q=article%2014",
    );
    ...
    const topPriority = await fetchJson(
      "http://127.0.0.1:4101/api/cases/priority?limit=5",
    );
    ...
    const detail = await fetchJson(
      `http://127.0.0.1:4101/api/cases/${firstId}`,
    );
    ...
    const knownCase = await fetchJson(
      "http://127.0.0.1:4101/api/cases/CASE-000001",
    );
    assert.equal(knownCase.status, 200);
    assert.ok(
      typeof knownCase.body.judgment === "string" &&
        knownCase.body.judgment.trim().length > 0,
    );
    assert.ok(
      typeof knownCase.body.final_verdict === "string" &&
        knownCase.body.final_verdict.trim().length > 0,
    );
