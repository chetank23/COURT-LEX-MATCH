...
async function main() {
  ...
  try {
    const extractedMode = await postJson("http://127.0.0.1:4102/api/analyze-pdf", {
    ...
    assert.ok(
      extractedMode.body.sections[0].highlights.includes("Text provided by upstream extractor"),
      "Expected extracted-text override path to be reflected in highlights"
    );
    assert.ok(
      extractedMode.body.sections[0].matches.some((item) => String(item.reason || "").toLowerCase().includes("citation")),
      "Expected at least one citation-aware ranking explanation"
    );

    const fallbackMode = await postJson("http://127.0.0.1:4102/api/analyze-pdf", {
    ...
    const nonLegalMode = await postJson("http://127.0.0.1:4102/api/analyze-pdf", {
    ...
    assert.ok(nonLegalMode.body.sections.every((section) => Array.isArray(section.matches) && section.matches.length === 0));
    assert.ok(
      String(nonLegalMode.body.sections[0].summary || "").toLowerCase().includes("no case matches"),
